...
Class Master extends DBConnection {
	...
	function save_client(){
		...
		foreach($_POST as $k =>$v){
			// chjgjgh
			if(in_array($k,array('client_code','fullname','status','password','address','company_name','email','contact','gender','tenure','website'))){
				if(!is_numeric($v))
				$v= $this->conn->real_escape_string($v);
				if(!empty($data)) $data .=", ";
				$data .=" `{$k}` = '{$v}' ";
		...
		if($save){
			...
			foreach($_POST as $k =>$v){
				// hghgghgh
				if(in_array($k,array('id','client_code','fullname','status','password','address','company_name','email','contact','gender','tenure','website')))
				continue;
				if(!empty($data)) $data .=", ";
				$data .= "('{$client_id}','{$k}','{$v}')";
			}
