    ...
    switch ($_GET["action"]) {
        case "add":
            if (! empty($_POST["quantity"])) {
                $productByCode = $db_handle->runQuery("SELECT * FROM school WHERE code='" . $_GET["code"] . "'");
                $itemArray = array(
        ...
        case "show_discount":
            if (! empty($_SESSION["cart_item"])) {
                if (! empty($_POST["discountCode"])) {
                    $priceByCode = $db_handle->runQuery("SELECT discunt FROM coupon_code WHERE coupon_code='" . $_POST["discountCode"] . "'");
